...
void Display::startRunningLine(const uint8_t* chars, uint8_t length, uint16_t stepDelay, uint8_t startPos, uint8_t endPos, bool loop, uint16_t loopPause) {
  ...
  _step = 0;
  _totalSteps = _startPos + (_length - 3) + (3 - _endPos);
  _lastStep = 0;
  _isRunning = true;
  _isNewLoop = false;
}
...
void Display::tick() {
  ...
  if (!_isNewLoop && millis() - _lastStep < _stepDelay) return;

  // Handle loop pause delay
...
void Display::showText(const String& str) {
  ...
  else startRunningLine(chars, str.length(), _defaultStepDelay, 0, 3, true);
}
